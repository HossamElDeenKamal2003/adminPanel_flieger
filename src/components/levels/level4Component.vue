<template>
    <div>
        <h1 class="text-center">Level Four</h1>
...
export default {
    ...
    methods: {
        async fetchPrices() {
            try {
                const response = await axios.get('http://localhost:3000/prices/level4/getprices');
        ...
        async handleSubmit() {
            if (this.editMode) {
                try {
                    const response = await axios.patch('http://localhost:3000/prices/level4/updateprices', this.form);
                ...
                try {
                    const response = await axios.post('http://localhost:3000/prices/level4/putprices', this.form);
                    alert(response.data.message);
                    this.fetchPrices();
                } catch (error) {
        ...
        deletePrice(country) {
            ...
            if (confirm(`Are you sure you want to delete prices for ${country}?`)) {
                ...
                axios.post('http://localhost:3000/prices/delete/level4', {
                    country: country
                }).then(() => {
                    this.fetchPrices();
                }).catch(error => {
